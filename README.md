# 🛡️ HackZero CTF 2026 – Writeups

Participated in **HackZero'26 (VIT Bhopal)** – a 24-hour Capture The Flag competition.

---

## 🏆 Performance

* **Team Name:** netscaf

* **Team Rank:** 31st

* **Team Score:** 4975 points

* **My Username:** Predator

* **My Contribution:** 2775 points

* **Total Solves (by me):** 21

---

## 📂 Repository Structure

This repository contains categorized writeups for all challenges **solved by me**, including:

* Step-by-step methodology
* Tools and techniques used
* Scripts (where applicable)
* Supporting files

---

## 🧠 Categories & Challenges (Solved by Me)

### 🔐 Cryptography

* VOIDMARKET
* An Open Secret
* Dead Signal
* Back to the Basics

---

### 🕵️ OSINT

* The Roadway in the Capital
* White Desert
* Tourist Kidnapped!
* Exit at 13,000 Feet
* The Anatomy of a Zombie : Level 1
* The Anatomy of a Zombie : Level 2
* The Anatomy of a Zombie : Level 3
* The Anatomy of a Zombie : Finale

---

### 🧬 Steganography

* Half Dust Half Deity
* Simple Stegno

---

### 🔍 Forensics

* Silent Signals in the Noise

---

### ☁️ Cloud Security

* Attachment Issues
* Can't Access??

---

### ⚙️ Miscellaneous

* Session Artifacts

---

### 🧾 Reversing

* Happy Birthday To me!

---

### 🤝 Sponsored Challenges

* Hackwithindia Sponsored Challenge
* Vibsecurity Sponsored Challenge

---

## 🛠️ Tools & Techniques Used

* Wireshark
* Burp Suite
* CyberChef
* Steghide / zsteg
* CrackStation / hashcat
* OSINT techniques (Google dorking, metadata analysis)
* Python scripting
* Reverse engineering tools

---

## 📘 Writeups

Each challenge folder contains:

* `writeup.md` → detailed explanation
* Scripts / payloads
* Relevant files

📄 A **49-page comprehensive report** is also included.

---

## ⚠️ Disclaimer

* Sensitive data and credentials have been **removed/redacted**
* Intended strictly for **educational purposes**

---

## 🚀 Key Learnings

* Real-world exploitation thinking
* Multi-domain cybersecurity exposure
* Working under time pressure in a competitive environment

---

## 📌 Author

**Khushi**
* ECE @ Central University of Jammu
* Cybersecurity Enthusiast

---
